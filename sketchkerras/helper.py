...
def show_active_img_and_save(name,img,path,show=False):
...
def show_img_and_save(name,img,path,show=False):
    mat = img.astype(np.float)
    if show:
        cv2.imshow(name,mat)
    cv2.imwrite(path,mat)
...
def show_active_img_and_save_denoise(name,img,path,show=False):
    mat = img.astype(np.float)
    mat = - mat + 1
    mat = mat * 255.0
    mat[mat < 0] = 0
...
def show_active_img_and_save_denoise_filter(name,img,path,show=False):
    mat = img.astype(np.float)
    mat[mat<0.18] = 0
    mat = - mat + 1
    mat = mat * 255.0
...
def show_active_img_and_save_denoise_filter2(name,img,path,show=False):
    mat = img.astype(np.float)
    mat[mat<0.1] = 0
    mat = - mat + 1
    mat = mat * 255.0
...
def show_active_img(name,img,show=False):
    mat = img.astype(np.float)
    mat = - mat + 1
    mat = mat * 255.0
    mat[mat < 0] = 0
